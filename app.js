...
class GoogleMap {
    ...
    async load (element){
        return new Promise((resolve, reject) =>{
            //$script来自vendor.js文件    $script 表示加载完第一个参数表示的文件后，再执行第二个参数表示的function
            ...
            this.textMarker = class TextMarker extends google.maps.OverlayView {
                ...
                hover() {
                    if(this.div !== null)
                        this.div.classList.add('.is-hover')
                }

                out() {
                    if(this.div !== null)
                        this.div.classList.remove('.is-hover')
                }
...
const initMap = async function() {
    ...
    Array.from(document.querySelectorAll('.item')).forEach(function (item) {
        let marker = this.map.addMarker(item.dataset.lat, item.dataset.lng, item.dataset.price + ' $')
